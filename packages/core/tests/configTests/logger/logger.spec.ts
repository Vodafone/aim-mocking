...
it('Should print only enabled logs', async () => {
  ...
  expect(loggerChecker.exists('[onProxyRequest]')).toBeGreaterThan(0)
  expect(loggerChecker.exists('[onProxyResponse]')).toBeGreaterThan(0)
  expect(loggerChecker.exists('[cacheController]')).toBeGreaterThan(0)
  expect(loggerChecker.exists('[cache]')).toBe(0)
  expect(loggerChecker.exists('[autofix]')).toBe(0)
