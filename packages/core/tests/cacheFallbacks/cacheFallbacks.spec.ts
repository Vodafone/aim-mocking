...
it('Should return root fallback mock', async () => {
  ...
  expect(loggerChecker.exists('tests/__mockapi__/__shared__/GET/api-test-1-000.json')).toBe(2)
  expect(loggerChecker.exists('exists: fallback')).toBe(1)
  expect(loggerChecker.exists('exists: fallback')).toBe(1)
...
it('Should return nested fallback mock', async () => {
  ...
  expect(loggerChecker.exists('tests/__mockapi__/a/b/__shared__/GET/api-test-1-000.json')).toBe(2)
  expect(loggerChecker.exists('exists: fallback')).toBe(1)
  expect(loggerChecker.exists('exists: fallback')).toBe(1)
  expect(loggerChecker.exists('cache served')).toBe(1)
}, 10000)
